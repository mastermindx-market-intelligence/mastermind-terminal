...
const OptionsRail = forwardRef<OptionsRailHandle, Props>(function OptionsRail({ children, symbol, enabled, access, pinned, onPin, replayActive }, ref) {
  ...
  const close = useCallback(() => { setOpen(false); onPin(null); launcher.current?.focus(); }, [onPin]);
  useImperativeHandle(ref, () => ({ open: () => { if (enabled) setOpen(true); } }), [enabled]);
  ...
  return <>
    <aside className="rail" data-options-rail={showing && !small ? "open" : "closed"}>
      {enabled && <div className={styles.railSwitch} role="group" aria-label={t("perspectives")}>
        <button type="button" aria-pressed={!showing} onClick={close}>{t("overview")}</button>
        <button type="button" ref={launcher} data-testid="options-rail-toggle" aria-pressed={showing} aria-label={t("open")} onClick={() => showing ? close() : setOpen(true)}>
    ...
    {enabled && <MobileSheet open={showing && small} onClose={close} title={`${t("options")} · ${symbol}`} ariaLabel={t("open")}
