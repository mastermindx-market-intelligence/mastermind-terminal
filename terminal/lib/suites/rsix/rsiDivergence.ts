// RSI Divergence — the shared 4-class divergence engine run over the RSI series, drawn in the RSI
// Ultimate pane suite ("rsix").
//
// Contract: lib/indicator-canvas/types.ts (frozen). Visual spec: docs/
// PREMIUM_INDICATOR_VISUAL_DESIGN_BIBLE_2026-07-28.md §"RSI Divergence — visual spec".
// Algorithm: docs/PREMIUM_INDICATOR_SUITE_MASTERPLAN_2026-07-28.md §8.3 "Divergence engine" (the
// detector is shared with the Pulse pane) and §8.4.
//
// This is a PANE module: every emitted price value is an RSI reading in the suite's own y-space
// (pane.min/max = 0..100), never a chart price. Bar indexing is unchanged.
//
//   rsi       = sharedRsi(ctx).rsi                           (the same series the Engine draws)
//   divs      = findDivergences(bars, rsi, { wing: 5, maxSpan: 60, hidden })
//   connector = straight line between the two RSI pivots, read off `rsi` by bar index — so the pane
//               line and the (future) price-pane mirror come from ONE detection record and can never
//               drift apart
//
// Visual grammar is deliberately IDENTICAL to the Pulse divergence module, so a trader who has read
// one pane can read the other without re-learning anything: regular = solid connector in the
// direction token, hidden = the same hue dashed at 0.6 alpha (continuation reads softer than
...
// two connectors (bible: cap connectors per anchor at 2).
//
...
import { MAX_EVENTS, boolOpt, clampNum, intOpt, sharedRsi } from "./rsiEngine";
import { RSI_DIVERGENCE_META } from "./rsiDivergence.meta";

// ------------------------------------------------------------------------------------ constants

const WING = 5; // RSI pivot half-window handed to the shared detector
...
function compute(ctx: ModuleCtx): ModuleResult {
  ...
  // ---- 1) the series this module reads (identical to the Engine's wave) ----------------
  // Engine settings come from ctx.suite, so a retuned length moves the pivots with the curve.
  const { rsi } = sharedRsi(ctx);
  if (rsi.length < n) return empty;

  // ---- 2) detect, normalize, order deterministically -----------------------------------
  const found = findDivergences(bars, rsi, { wing: WING, maxSpan: MAX_SPAN, hidden: wantHidden });
  const rows: Div[] = [];
  for (const e of found) {
    ...
    if (!Number.isFinite(rsi[ai]) || !Number.isFinite(rsi[bi])) continue;
  ...
  const L = {
    title: zh ? "RSI 背离" : "RSI divergence",
    cls: zh ? "类型" : "Class",
    span: zh ? "跨度" : "Span",
    strength: zh ? "强度" : "Strength",
    stack: zh ? "叠加" : "Stacked",
    bars: zh ? "根" : "bars",
    osc: zh ? "RSI 变化" : "RSI travel",
  };
  ...
  for (const g of drawn) {
    ...
    const tipId = `rsix-div-${g.confirmedAt}-${g.bi}-${g.dir}`;

    for (const d of fan.slice(0, MAX_FAN)) {
      prims.push({
        kind: "poly",
        id: `rsix-div-c-${d.ai}-${d.bi}-${d.kind}`,
        z: 2,
        pts: [
          { i: d.ai, p: rsi[d.ai] },
          { i: d.bi, p: rsi[d.bi] },
...
export const RSI_DIVERGENCE_MODULE: SuiteModuleDef = { ...RSI_DIVERGENCE_META, compute };

export default RSI_DIVERGENCE_MODULE;
