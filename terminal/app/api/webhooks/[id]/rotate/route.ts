...
import { rotateEndpointSecret, WEBHOOK_ROUTE_MESSAGES, type WebhookRouteCode } from "@/lib/webhooks";
import type { TenancyRpcDb } from "@/lib/teams";
...
export async function POST(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const result = await rotateEndpointSecret(session.db, session.userId, id);
  if (!result.ok) {
    return NextResponse.json(errorBody(result.code.toUpperCase(), result.code), { status: result.status });
  }
  return NextResponse.json({ ok: true, secret: result.secret, secretVersion: result.secretVersion });
}
