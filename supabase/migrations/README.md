...
**Which prefixes carry a vitest ledger pin.** Every prefix from `0017` to `0023` and `0026` now has a vitest contract under `terminal/lib/__tests__/` that reads `RESERVATIONS.json` and asserts its row's `state`, `pr`, `pr_state`, `merged_sha` and applied fields, plus the agreement between that row and the `-- Ledger row:` header line of the `.sql` it names: `0017` in `personalAccuracyMigrationContract.test.ts`, `0021` in `resourceGrantsMigrationContract.test.ts`, and `0018`, `0019`, `0020`, `0022`, `0023` and `0026` in the `*LedgerContract.test.ts` files added beside them (`0026` lands in `webhookRotationLedgerContract.test.ts`). They are a second reader, not the authority: `tests/test_supabase_migration_namespace.py` remains the **primary** pin — it is what CI's python job runs, it carries the STRICT / PULL_REQUEST / LENIENT scope rules described below, and it covers every prefix in this ledger, including the ones below `0017` that no vitest file names.
