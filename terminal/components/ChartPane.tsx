...
export default function ChartPane({ idx, symbol, drawingOwnerKey, isActive, onActivate, row, tf, chartType, dataReady = true, initialTimeframe = null, inds, tool, toolActivation = 0, drawingSticky = false, drawingCreationDisabled = false, hostDisplay, drawStyle, detectCmd, compare, compareCfg, magnet, replayIdx, onMeta, drawings, drawingsVisible = true, onDrawingsChange, liveQuote, indParams, hidden, onToggleHidden, onRemoveInd, onOpenSettings, onOpenSource, pineScripts,
  ...
  return (
    <div className={`pane${isActive ? " on" : ""}${swapping ? " is-swapping" : ""}`} data-swapping={swapping ? "1" : undefined} style={hostDisplay ? { display: hostDisplay } : undefined} onPointerDownCapture={() => { if (!isActive) onActivate(idx); }}>
