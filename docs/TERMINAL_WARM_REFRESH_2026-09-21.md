...
## Integration-run correction — 2026-09-22

GitHub's existing controller joined path-disjoint options touch-target master into this carrier at d26da0838650b11a47fe3cf613b0b560ace33530. The production cache source remained byte-identical. The original semantic-head CI35675400380 passed; the newer integration run35678327447 failed only its desktop warm-picker case, before reaching any cache assertion: its visual-ready predicate stayed false with the test's wall clock already frozen during initial chart hydration. Mobile, tablet, serial, unit, hub and ingest checks passed. Required CI remains blocking.

The test now leaves initial chart hydration on the real clock, observes the same chart-ready and cached-company preconditions, then advances wall time90seconds from the observed warmed time solely for cache expiry. No source behavior, readiness assertion, data expectation, timeout, retry count, request count or production data was weakened or changed. This removes unnecessary startup-clock coupling from the test; it is not yet proof that every possible readiness failure shares that cause.

Two fresh runs of the actual repaired browser journey passed with retries disabled. Both required cached company before releasing the held refresh and corrected company afterwards, with exactly one initial request and one refresh. Hosted qualification of the repaired exact head remains owed. The failed original artifact is retained under /Volumes/Mastermind/evidence/terminal-707-desktop-35678327447.
