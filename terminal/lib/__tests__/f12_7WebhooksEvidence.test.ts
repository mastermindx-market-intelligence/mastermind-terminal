...
describe("B-F12-7 evidence lock is the sha256 of the layout sources", () => {
  it("no test file this packet adds imports a process spawner", () => {
    ...
    // Exact packet test-file count at this head: 15 lib/__tests__ files matching
    // webhook*|f12_7Webhooks* (the packet's 13 plus
    // webhookDeliveryLedgerContract.test.ts, plus
    // webhookRotationMigrationContract.test.ts from B-F12-11), plus
    // SectionWebhooks.test.tsx (globbed from components/settings/__tests__),
    // plus the e2e spec = 17. An equality keeps a broken glob from passing
