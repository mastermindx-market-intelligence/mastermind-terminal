...
const COPY = {
  en: {
    eyebrow: "RESEARCH WORKSPACE", title: "Theses", newThesis: "New thesis", list: "Your theses",
    ...
    horizon: "Horizon", effective: "Effective as of (optional)", revision: "Revision note",
    ...
    retrySame: "Retry same request", saved: "Saved as version", lines: "Complete snapshot",
  },
  zh: {
    eyebrow: "研究工作区", title: "研究论点", newThesis: "新建论点", list: "你的论点",
    ...
    horizon: "时间范围", effective: "生效时间（可选）", revision: "修订说明",
    ...
    retrySame: "重试同一请求", saved: "已保存为版本", lines: "完整快照",
  },
} as const;
...
export default function ThesisWorkspace({ initialSymbol, initialThesisId, invalidLink = false }: ThesisWorkspaceProps) {
  const { lang } = useLang();
  ...
  const loadList = useCallback(async () => {
    try {
      ...
      if (!response.ok) return setListState("unavailable");
      const payload = await response.json();
      if (!Array.isArray(payload.theses)) return setListState("unavailable");
  ...
  useEffect(() => {
    ...
    if (initialThesisId) void Promise.resolve().then(() => loadDetail(initialThesisId));
  }, [initialThesisId, invalidLink, loadDetail, loadList]);
  ...
  const consumeResponse = useCallback(async (response: Response, action: ThesisAction) => {
    const payload = await response.json().catch(() => ({}));
    setSaving(false);
    setPending(null);
    if (response.status === 401) return setListState("session_expired");
    if (response.status === 409 && payload.error === "version_conflict") {
      setConflict({ currentVersion: payload.currentVersion, lifecycleState: payload.lifecycleState });
      return;
    }
    if (!response.ok) {
    ...
    const id = payload.thesisId as string;
    const url = new URL(window.location.href);
    url.searchParams.set("view", "theses");
    url.searchParams.set("thesis", id);
    url.searchParams.delete("symbol");
    window.history.replaceState(null, "", url.toString());
    setMessage(`${copy.saved} ${payload.version}${payload.replayed ? " · replayed" : ""}`);
    await loadList();
    await loadDetail(id);
    if (action === "create") setSelectedId(id);
  }, [copy.invalid, copy.saved, copy.transition, loadDetail, loadList]);
  ...
  const send = useCallback(async (pendingMutation: Pending) => {
    ...
    try {
      ...
      await consumeResponse(response, pendingMutation.action);
    } catch {
      setSaving(false);
  ...
  return (
    <main className={`main2 ws-shell ${styles.root}`} data-testid="thesis-workspace" data-list-state={listState}>
      <header className={styles.contextBar}>
        <div><small>{copy.eyebrow}</small><h1>{copy.title}</h1></div>
      ...
      {listState === "session_expired" ? (
        ...
        <div className={styles.workspaceGrid}>
          <aside className={styles.rail} aria-label={copy.list}>
            ...
            {truncated && <p className={styles.muted}>More theses exist; refine this workspace before continuing.</p>}
          </aside>
                ...
                : detailState === "unavailable" ? <div className={styles.centerState} role="status"><span className={styles.stateMark}>!</span><h1>{copy.unavailable}</h1><p>{copy.unavailableBody}</p>{selectedId && <button onClick={() => beginDetailLoad(selectedId)}>{copy.retry}</button>}</div>
                  : <>
                    <div className={styles.editorHeader}>
                      <div><small>{detail ? statusLabel(lifecycle, copy) : copy.newThesis}</small><h1>{detail?.subject.display ?? (subjectDraft || copy.newThesis)}</h1><p>{detail?.subject.identityState === "listing_scoped" || !detail ? copy.listingScoped : detail.subject.owner}</p></div>
                    ...
                    <form className={styles.form} onSubmit={(event) => { event.preventDefault(); submit(selectedId ? "revise" : "create"); }}>
                      <label>{copy.subject}<input aria-label={copy.subject} value={detail?.subject.key ?? subjectDraft} disabled={!!detail} onChange={(event) => { setSubjectDraft(event.target.value.toUpperCase()); setPending(null); }} placeholder="NVDA" /></label>
                      <label>{copy.titleLabel}<input aria-label={copy.titleLabel} value={draft.title} disabled={!editable} maxLength={160} onChange={(event) => changeDraft("title", event.target.value)} /></label>
                      <label className={styles.full}>{copy.statement}<textarea aria-label={copy.statement} value={draft.statement} disabled={!editable} maxLength={12000} rows={8} onChange={(event) => changeDraft("statement", event.target.value)} /></label>
                      {(["catalysts", "falsifiers", "risks"] as const).map((field) => <label key={field}>{copy[field]}<small>{copy.onePerLine}</small><textarea aria-label={copy[field]} value={draft[field]} disabled={!editable} rows={5} onChange={(event) => changeDraft(field, event.target.value)} /></label>)}
                      <label>{copy.horizon}<select aria-label={copy.horizon} value={draft.horizon} disabled={!editable} onChange={(event) => changeDraft("horizon", event.target.value as ThesisHorizon)}>{["unspecified", "days", "weeks", "months", "quarters", "years"].map((value) => <option key={value}>{value}</option>)}</select></label>
                      <label>{copy.effective}<input aria-label={copy.effective} type="datetime-local" value={draft.effectiveAt} disabled={!editable} onChange={(event) => changeDraft("effectiveAt", event.target.value)} /></label>
                      <label className={styles.full}>{copy.revision}<textarea aria-label={copy.revision} value={draft.revisionNote} maxLength={1000} rows={3} onChange={(event) => changeDraft("revisionNote", event.target.value)} /></label>
                      <div className={`${styles.actions} ${styles.full}`}>
                        {editable && <button className={styles.primaryButton} type="submit" disabled={saving}>{saving ? copy.saving : copy.save}</button>}
                        {detail && lifecycle === "active" && <><button type="button" disabled={saving} onClick={() => submit("archive")}>{copy.archive}</button><button type="button" className={styles.dangerButton} disabled={saving} onClick={() => submit("invalidate")}>{copy.invalidate}</button></>}
                        {detail && lifecycle !== "active" && <button type="button" className={styles.primaryButton} disabled={saving} onClick={() => submit("reopen")}>{copy.reopen}</button>}
                      </div>
                    </form>

                    {detail && <section className={styles.history} aria-label={copy.history}><div className={styles.historyHeading}><h2>{copy.history}</h2><span>{detail.history.length}</span></div>{history.map((entry) => <article key={entry.id} data-version={entry.version}><div><b>v{entry.version}</b><span>{entry.transition}</span><i data-state={entry.lifecycleState}>{statusLabel(entry.lifecycleState, copy)}</i></div><time dateTime={entry.systemRecordedAt}>{new Date(entry.systemRecordedAt).toLocaleString(lang === "zh" ? "zh-CN" : "en-CA")}</time><strong>{entry.content.title}</strong><p>{entry.content.revisionNote || copy.lines}</p></article>)}{detail.historyTruncated && <p className={styles.muted}>History is truncated; no versions were silently discarded.</p>}</section>}
                  </>}
          </section>
        </div>
      )}
