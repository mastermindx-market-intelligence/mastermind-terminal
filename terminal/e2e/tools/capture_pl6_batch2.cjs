 ...
 *
 * Writes PNGs + EVIDENCE.yml under docs/pr-crops/b-pl-6-batch-2/.
 */
"use strict";
...
async function captureGuidePanel(page, width, lang, outPath) {
  ...
  await cropLocator(page, guide, outPath, 10);
}
...
async function captureLevelsView(page, width, lang, outPath) {
  ...
  const labeled = page.getByText(needle).first();
  if (await labeled.count()) {
    await labeled.waitFor({ state: "visible", timeout: 20_000 }).catch(() => {});
  }
  if (width === 390) {
